...
```mermaid
graph LR
  Sensor[Камера IMX219] -- CSI Bus --> RPi[RPi 3B / Custom Linux Buildroot]
  RPi -- UART 115200 bps --> STM[STM32F103 CMSIS]
  STM -- PWM CH1 --> Pan[Серво Pan]
  STM -- PWM CH2 --> Tilt[Серво Tilt]
```
...
2. **Анализ (Vision Processing)**: aspberry Pi 3B выполняет инференс нейросети. Вычисляется вектор ошибки $e(t)$ относительно центра кадра. Применяется фильтрация (например, фильтр Калмана) для сглаживания траектории цели.
...
1. High-Level Unit (Computer Vision Node): - База: Raspberry Pi 3b.

       ОС: Turret OS (собственный дистрибутив на базе Buildroot).

       Задачи: Кросс-компиляция системных компонентов на Arch Linux (Host), оптимизация инференса нейросетей под архитектуру ARM64.
...
**[Даниил Светашов] — Team Lead & Embedded Linux Engineer**
  ...
  - Развертывание и глубокое конфигурирование Raspberry Pi 3B.

  - Настройка процесса кросс-компиляции (Toolchain) на хост-системе Arch Linux.
    
  - Проектирование и сборка кастомной операционной системы через Buildroot.
